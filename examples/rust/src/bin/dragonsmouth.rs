...
impl BlockConstruction {
    ...
    fn try_seal(&mut self) {
        if let Some(i) = self.block_meta_vec.last() {
            ...
            if let BlockEvent::BlockMeta(block_meta) = boxed_event {
                ...
                let total_account_writes: usize = self
                    .tx_account_writes_index
                    .values()
                    .map(|set| set.len())
                    .sum();
                ...
                if entry_count_match
                    && tx_count_match
                    && account_update_match
                    && entry_nested_count_match
                {
                    for (sig1, pubkeyset_1) in self.account_update_tx_sig_index.iter() {
                        let pubkeyset_2 =
                            if let Some(pubkset_2) = self.tx_account_writes_index.get(sig1) {
                                pubkset_2
                            } else {
                                self.error = Some(BlockConstructionError::UnexpectedAccountUpdate);
                                return;
                            };
                        if pubkeyset_1 != pubkeyset_2 {
                            self.error = Some(BlockConstructionError::UnexpectedAccountUpdate);
                            return;
                        }
                    }
                    ...
                    println!(
                        "Block sealed tx_account_writes={}, account_writes={:?}, entry count={}",
                        total_account_writes,
                        self.account_vec.len(),
                        block_meta.entries_count
                    );
                    self.is_sealed = true;
                } else {
                    let missing_tx =
                    ...
                    eprintln!(
                        "Block not sealed, empty_tx_sig_account_update={}, failed_tx={}, missing tx={}, missing entries={}, unknown account tx sigs = {}, missing account updates = {}", 
    ...
    fn add_event_checked(&mut self, event: BlockEvent) -> Result<(), InvalidBlockEvent> {
        ...
        match &event {
            ...
            BlockEvent::Account(subscribe_update_account) => {
                self.handle_add_account(time, subscribe_update_account);
            }
            _ => {}
        }
    ...
    fn handle_add_account(&mut self, time: u32, account: &SubscribeUpdateAccount) {
        ...
        if let Some(sig) = account.txn_signature() {
            ...
            self.emptysig_accounts_index.push(time as usize);
        }
    }
